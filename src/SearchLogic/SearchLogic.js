...
nsGmx.searchLogic = {
    showResult: function (response, oMenu) {
        console.log(response);
        var oSearchResultDiv = document.createElement('div');
        oSearchResultDiv.className = 'ddfdfdf';

        console.log(oSearchResultDiv);

        if (oMenu != null){
            var alreadyLoaded = oMenu.createWorkCanvas("search");
            if(alreadyLoaded) {
                $(oMenu.workCanvas).empty();
            };
            oMenu.workCanvas.appendChild(oSearchResultDiv);
        }

    	var oRenderer = new ResultRenderer(nsGmx.leafletMap, imagesHost, true);

        var lstResult = new ResultList(oSearchResultDiv, imagesHost);
        ...
        var fnDisplayedObjectsChanged = function(event, iDataSourceN, arrFoundObjects){
            ...
            @param {int} iDataSourceN № источника данных(группы результатов поиска)
            @param {object[]} arrDSDisplayedObjects Результаты поиска, которые необходимо отобразить в текущей группе*/
            // $(oSearchResultDiv).triggerHandler('onDisplayedObjectsChanged', [iDataSourceN, arrFoundObjects]);
            $(lstResult).triggerHandler('onDisplayedObjectsChanged', [iDataSourceN, arrFoundObjects]);
        }
        ...
        // $(oSearchResultDiv).bind('onDisplayedObjectsChanged', fnDisplayedObjectsChanged);
        // $(oSearchResultDiv).bind('onObjectClick', fnObjectClick);
        // $(oSearchResultDiv).bind('onDownloadSHP', fnDownloadSHP);
