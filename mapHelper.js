...
mapHelper.prototype.createLayerEditorProperties = function(div, type, parent, properties)
{
	...
	if (!haveTask)
	{
		...
		if (div)
			saveButton = makeLinkButton(_gtxt("Изменить"));
		else
			saveButton = makeLinkButton(_gtxt("Создать"));
...
// mapHelper.prototype.createAPIMap = function(hostName, mapName, key)
// {

	// var id = 'flash' + Math.random(),
		// br = ($.browser.msie) ? "\n\r" : "\n",
		// // hostName = _mapHelper.mapProperties.hostName,
		// // mapName = _mapHelper.mapProperties.name,
		// mapText = "<div>" + br + "\t<div id=\"" + id + "\" style=\"width: 600px; height: 400px; overflow:hidden;\"></div>" + br;
	
	// if (key)
		// mapText += "\t<script charset=\"windows-1251\" src=\"http://maps.kosmosnimki.ru/api/api.js?key=" + key + "\"></script>" + br;
	// else
	...
	// if (globalFlashMap.getBaseLayer())
		// mapText += "\t\t\tmap.setBaseLayer(\"" + globalFlashMap.getBaseLayer() + "\");" + br;
	
	// mapText += "\t\t\tmap.minimizeTools();" + br;
	// mapText += "\t\t\tmap.grid.setVisible(" + _mapHelper.gridView + ");" + br; //FIXME: use API
	...
	// {
		...
		// var color = drawnObjects[i].color || 0x0000FF,
			// thickness = drawnObjects[i].thickness || 3,
			// opacity = drawnObjects[i].opacity || 80,
			// geometry = from_merc_geometry(drawnObjects[i].geometry)
			// elemId = 'elem' + String(Math.random()).split(".")[1];
		
		// mapText += "\t\t\tvar " + elemId + " = map.drawing.addObject(" + JSON.stringify(geometry) + ", " + JSON.stringify(drawnObjects[i].properties) + ")" +  br;
		// mapText += "\t\t\t" + elemId + ".setStyle({outline: {color: " + color + ", thickness: " + thickness + ", opacity: " + opacity + " }, marker: { size: 3 }, fill: { color: 0xffffff }}, {outline: {color: " + color + ", thickness: " + (thickness + 1) + ", opacity: " + Math.min(100, opacity + 20) + "}, marker: { size: 4 }, fill: { color: 0xffffff }})" + br;
	// }

	
	// _mapHelper.findChilds(window.oldTree, function(child, flag)
	// {

		// var layer = globalFlashMap.layers[child.content.properties.name];
		
		// if (typeof layer.isVisible != 'undefined' && layer.isVisible != flag)
			// mapText += "\t\tmap.layers[\"" + child.content.properties.name + "\"].setVisible(" + layer.isVisible + ")" +  br;
	// }, true)
	// // for (k = 0; k < globalFlashMap.layers.length; k++)
	// // {
		// // var layer = globalFlashMap.layers[k];
		// // var name = layer.properties.name || layer.properties.image;
		// // mapText += "\t\tmap.layers[\"" + name + "\"].setVisible(" + layer.isVisible + ")" +  br;
	// // }
...
mapHelper.prototype.createAPIMapDialog = function()
{
	var options = {
			...
			/*requestTerms: false,
			termsURL: 'http://kosmosnimki.ru/terms.html'*/
		};
...
// mapHelper.prototype.createAPIMapDialog = function(mapName, hostName)
// {
	// var input = _input(null, [['dir','className','inputStyle'],['css','width','200px']]),
		...
		// canvas = _div([_div([_span([_t(_gtxt("Введите API-ключ"))],[['css','fontSize','12px'],['css','margin','0px 7px']]), getAPI, _div([input, button], [['css','margin','10px 0px 10px 5px']])]), mapTextArea]),
		// //mapName = _mapHelper.mapProperties.name,
		// //hostName = _mapHelper.mapProperties.hostName,
		// inputError = function()
		// {
	...
	// if (!(this.mapProperties.UseKosmosnimkiAPI ||
		// this.mapProperties.hostName == "maps.kosmosnimki.ru") || !window.apiKey)
	// {
		...
		// resize();
	  	...
	  	// mapTextArea.value = _this.createAPIMap(mapName, hostName);
		// mapTextArea.select();
	// }
		...
		// {
			...
			// {
				...
				// {
					...
					// mapTextArea.value = _this.createAPIMap(mapName, hostName, input.value);
