(function () {
    var tzm = (new Date()).getTimezoneOffset() * 60000,
		...
		getTimeString = function(dt) {
			...
			[
				pad2(dt.getHours()),
				pad2(dt.getMinutes() + (dt.getSeconds() > 0 ? 1 : 0))
			].join(':');
		};
...
links.Timeline = function(container, options) {
    ...
    this.getUTCTimeString = getUTCTimeString;
...
links.Timeline.prototype.repaintCurrentTime = function() {
    ...
    dom.currentTime.title = "Current time: " + getUTCTimeString(now);
...
links.Timeline.prototype.zoom = function(zoomFactor, zoomAroundDate) {
    ...
    var newEnd   = new Date(this.end.valueOf() - endDiff * zoomFactor - 0);
