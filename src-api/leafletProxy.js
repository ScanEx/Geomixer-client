...
(function()
{
	...
	var nextLoad = function()	{		// загрузка image
		...
		if(item.bounds && !gmxAPI._leaflet['zoomstart']) {						// удаление устаревших запросов по bounds
...
(function()
{
	...
	var utils = {							// Утилиты leafletProxy
		...
		,
		'chkGlobalClick': function(attr)	{					// проверка Click на перекрытых нодах
			for (var i = 0; i < gmxAPI.map.layers.length; i++)
			{
				var child = gmxAPI.map.layers[i];
				if(!child.isVisible) continue;
				var mapNode = mapNodes[child.objectId];
				if(mapNode['eventsCheck'] && mapNode['eventsCheck']('onClick', attr)) return;
			}
		}
		,
		...
		'drawPoint': function(node, style)	{			// отрисовка POINT геометрии
			...
			if(styleType === 'marker') {						// стиль маркер
				...
				if(!style['scale']) style['scale'] = 1;
					...
					opt['iconAnchor'] = (style['center'] ? new L.Point(ww/2, hh/2) : new L.Point(0, hh));
					//opt['iconAnchor'] = new L.Point(ww/2, hh/2);
				//}
		...
		if(node['leaflet']) {
			...
			if(evName === 'onClick') {
				var func = function(e) {		// Проверка click
					//if(gmxAPI._leaflet['skipClick']) return;
					hNode['handlers'][evName](id, node.geometry.properties, {'ev':e});
				};
...
*/			
		...
		'setZoomBounds':	function(ph)	{		// Установка границ по zoom
			...
			if(node.propHiden) {
				...
				} else if(node['type'] == 'mapObject') {			//ограничение по зуум mapObject
					//var zoom = LMap.getZoom();
					//var flag = ((node['minZ'] && zoom < node['minZ']) || (node['maxZ'] && zoom > node['maxZ']) ? false : true);
					//if(!flag) setVisibleRecursive(node, flag);
					//gmxAPI._listeners.addListener({'level': -10, 'eventName': 'onZoomend', 'obj': gmxAPI.mapNodes[id], 'func': function() {
...
*/			
		...
		node['eventsCheck'] = function(evName, attr) {			// проверка событий векторного слоя
			...
			for (var tileID in node['tiles'])
			{
				...
				if(tileBound.contains(attr.mPoint)) {
...
//							if(bounds.intersects(pt))	{			// Тайл пересекает границы - необходимо загрузить
			...
			if(evName === 'onClick' && arr.length) {
				...
				//gmxAPI._leaflet['skipClick'] = true;
				//setTimeout(function() { gmxAPI._leaflet['skipClick'] = false; }, 100);
...
//utils.bringToDepth(node, node['zIndex']);
		...
		function styleToGeo(geo, filter)	{			// Стиль обьекта Получить фильтры для обьекта
			var style = (filter ? utils.evalStyle(filter.regularStyle, geo)
				: {													// стиль Canvas по умолчанию
					'stroke': {
						'weight': 1
						,'opacity': 1
...
var tt =1;
		...
		node['addItems'] = function(attr) {			// добавление обьектов векторного слоя
			var arr = [];
			var data = attr.data;
			for (var z in data)
			{
				for (var x in data[z])
				{
					for (var y in data[z][x])
					{
						var tileID = z + '_' + x + '_' + y;
						var pt = getTilesBounds([Math.floor(x), Math.floor(y), Math.floor(z)]);
						if(!node['tiles'][tileID]) node['tiles'][tileID] = pt[tileID];
						node['tilesLoadProgress'][tileID] = true;	// Тайл не надо грузить с сервера
						objectsToFilters(data[z][x][y], tileID);
					}
				}
			}
			return true;
...
//console.log('dddddd ' , arr.length , ' : ' , attr);
				...
				for (var i1 = 0; i1 < arr.length; i1++)
				{
					...
					if(filters && filters.length) {
						...
						if(filter && filter.styleRecalc) styleToGeo(geom, filter);
					}
...
if(geom['properties']['Magnitude']) {
tt = 1;
}
					if(!geom.curStyle || !node.chkTemporalFilter(geom)) continue;
/*
...
*/
					if(geom.type === 'Point' || geom.type === 'Polyline' || geom.type === 'MultiPolyline' || geom.type === 'Polygon' || geom.type === 'MultiPolygon') {
						if(geom['intersects']) {
							if(!geom['intersects'](attr.bounds)) continue;	// если geom имеет свой метод intersects
						}
						else if(!attr.bounds.intersects(geom.bounds)) continue;					// обьект не пересекает границы тайла
...
*/								
			...
			for (var key in node['tilesGeometry'])						// Перебрать все загруженные тайлы
			{
				...
				if(!tb) continue;
				if( tb.min.x - tb.delta > attr.bounds.max.x
...
//console.log('waitMe ' , pos);
		//LMap.on('moveend', function(e) { gmxAPI._updatePosition(e); });
			...
			//LMap.on('mousedown', function(e) {		// Проверка click слоя
				var attr = {
				...
				/*if(attr['_layer']) {
					...
					gmxAPI._leaflet['utils'].chkGlobalClick(attr);
				//}
			});
...
*/
							if(!flagAll) {
								...
								drawCanvasPolygon( ctx, tileX, tileY, attr['geom'], zoom, bounds );
							}
...
*/
					...
					for (var key in tiles)
					{
						//if(!node['dataTiles'][st]) {
							var pt = tiles[key];
//console.log(drawTileID + ' : __________' + key , bounds, pt);
							if(bounds.intersects(pt))	{			// Тайл пересекает границы - необходимо загрузить
								var arr = key.split('_');
								var tx = Number(arr[1]);
								var ty = Number(arr[2]);
								...
								if(!node['tilesGeometry'][sst] && !node['badTiles'][sst] && !node['tilesLoadProgress'][sst]) {
