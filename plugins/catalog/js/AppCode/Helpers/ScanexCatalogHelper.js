...
(function($){
	var ScanexCatalogHelper = function() {
		this.satellites = [
			// local data
				{ source: 'scanex', name: 'Сканэкс', restricted: true,
					options: {
						title: 'Спутники',
						items: {
							...
							'WV03': {id: 'WV03', name: 'WorldView-3', source: 'search', resolution: 0.5, color: 0x800000, checked: true },
							'GE-1': {id: 'GE-1', name: 'GeoEye-1', source: 'search', resolution: 0.5, color: 0x0000ff, checked: true},
							'Pleiades': {id: '1B_PM,1A_PM', name: 'Pleiades A-B', source: 'search', resolution: 0.5, color: 0x0000ff, checked: true },
							'QB02': {id: 'QB02', name: 'QuickBird', source: 'search', resolution: 0.6, color: 0x808080, checked: true },
							'EROS-A': {id: 'EROS-A1', name: 'EROS-A', source: 'search', resolution: 0.7, color: 0x008080, checked: true },
							...
							'SPOT 6': {id: 'SPOT 6,SPOT6,SPOT 7,SPOT7', name: 'SPOT-6,7', source: 'search', color: 0x006400, checked: true},
							'LANDSAT_8': {id: 'LANDSAT_8', name: 'LANDSAT 8', source: 'search', resolution: 15, color: 0x0000ff, checked: true }
						}
					}
				}
				// commercial data
				, {source: 'external', name: 'Коммерческие данные',
					options: {
						title: 'Спутники',
						items: {
							'GE-1': {id: 'GE-1', name: 'GeoEye-1', source: 'search', resolution: 0.5, color: 0x0000ff, checked: true},
				...
				, {source: 'public', name: 'Открытые данные',
					options: {
						title: 'Спутники',
						items: {
							'LANDSAT_8': {id: 'LANDSAT_8', name: 'LANDSAT 8', source: 'search', resolution: 15, color: 0x0000ff, checked: true }
	...
	ScanexCatalogHelper.prototype.findSpot5 = function(items, product){
		for(var id in items){
			var item = items[id];
	...
	ScanexCatalogHelper.prototype.findSatelliteById = function(source, id, product) {
	...
	nsCatalog.Helpers.ScanexCatalogHelper = new ScanexCatalogHelper();
