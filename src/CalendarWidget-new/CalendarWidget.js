...
var Calendar = nsGmx.GmxWidget.extend({
    ...
    showCalendar: function () {
        var _this = this,
            ...
            dayms = nsGmx.DateInterval.MS_IN_DAY,
            oneDayPeriod = (dateEnd.valueOf() - dateBegin.valueOf() === dayms),
            beginCalendar = $(this.calendarTemplates.beginTemplate({oneDayPeriod: oneDayPeriod})),
            endCalendar = $(this.calendarTemplates.endTemplate({})),
            createIntervalButton = $('.createdateinterval-button', beginCalendar),
            resetIntervalButton = $('.resetdateinterval-button', beginCalendar),
            selectIntervalButton = $('.selectdateinterval-button', endCalendar),
            beginDialogOptions = {
                dialogClass: "calendar-outside begin-calendar",
                draggable: false,
                resizable: false,
        ...
        this._dateBegin = $('.begin-outside-calendar', beginCalendar);
        this._dateEnd = $('.end-outside-calendar', endCalendar);
        ...
        this._dateBegin.datepicker('setDate', Calendar.toUTC(dateBegin));
        this._dateEnd.datepicker('setDate', Calendar.toUTC(new Date(dateEnd.valueOf() - dayms)));

        $(beginCalendar).dialog(beginDialogOptions);

        if (this.getMode() === Calendar.ADVANCED_MODE) {
            $(createIntervalButton).toggle(false);
            $(resetIntervalButton).toggle(true);
            $(endCalendar).dialog(endDialogOptions);
        }
        ...
        $(resetIntervalButton).on('click', function () {
            ...
            $(createIntervalButton).toggle(true);
            _this._dateInterval.set({
                dateEnd: new Date(begin.valueOf() + dayms)
            })
        })
    ...
    _selectFunc: function(activeInput) {
        ...
        if (this._curMode === Calendar.SIMPLE_MODE) {
            ...
            var _begin = toMidnight(this._dateInterval.get('dateBegin')),
                _end = toMidnight(this._dateInterval.get('dateEnd'));

            // если дата в итоге не поменялась
            if (begin.valueOf() === _begin.valueOf() && (end.valueOf() + dayms) === _end.valueOf()) {
                $(selectIntervalButton).addClass('disabled');
            } else {
                $(selectIntervalButton).removeClass('disabled');
            }
    ...
    _updateModel: function() {
        var dateBegin = this.getDateBegin(),
            dateEnd = this.getDateEnd(),
            sameDay = dateBegin.valueOf() === dateEnd.valueOf();

        dateEnd = sameDay ? new Date(dateBegin.valueOf() + nsGmx.DateInterval.MS_IN_DAY) : new Date(dateEnd.valueOf() + nsGmx.DateInterval.MS_IN_DAY);
